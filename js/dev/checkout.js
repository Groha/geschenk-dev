...
const utilsScriptUrl = "" + new URL("../assets/intl/utils.js", import.meta.url).href;
const cart = getCart();
if (!cart.length) {
...
if (phoneInput) {
  let updateLabel = function() {
    ...
    formField.classList.toggle("focused", hasValue || isFocused);
  };
  const formField = phoneInput.closest(".form-field");
...
if (zipInput && cityInput) {
  zipInput.addEventListener("input", () => {
    zipInput.value = zipInput.value.replace(/[^\d]/g, "").trim();
    if (zipInput.value.length === 5) {
      fetch(`https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&q=${zipInput.value}&countrycodes=DE&limit=1`).then((res) => res.json()).then((data) => {
        if (data.length > 0 && data[0].address) {
          const place = data[0].address;
          cityInput.value = place.city || place.town || place.village || "";
        } else {
          cityInput.value = "";
        }
      }).catch(() => {
